...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    Node searchForNode(Key key){
        return searchForNode(key, root);
    }
    Node searchForNode(Key key, Node x){

        if(key == null){
            throw new IllegalArgumentException("calls search() with a null key");
        }
        int cmp = key.compareTo(x.key);
        if(cmp < 0)return searchForNode(key, x.left);
        else if(cmp > 0)return searchForNode(key, x.right);
        else return x;
    }

    private void delete(Node will_be_deleted){
        if(will_be_deleted.parent == null) {root = null; return;}
        Node parentOfWillBeDeleted = will_be_deleted.parent;
        if(parentOfWillBeDeleted.right == will_be_deleted)parentOfWillBeDeleted.right = null;
        else parentOfWillBeDeleted.left = null;
    }
    // deletes node with key key from tree
    public void delete(Key key) {
		Node will_be_deleted = searchForNode(key);
		if(will_be_deleted.left == null && will_be_deleted.right == null){
		    this.delete(will_be_deleted);
        }
        else if(will_be_deleted.left == null){
		    transplant(will_be_deleted,will_be_deleted.right);
        }
        else if(will_be_deleted.right == null){
		    transplant(will_be_deleted,will_be_deleted.left);
        }
        else{
		    Node tmp = successor(will_be_deleted);
		    tmp.right = will_be_deleted.right;
		    tmp.left = will_be_deleted.left;
		    transplant(will_be_deleted,tmp);
        }
